...
if os.path.exists(diag_json_path):
    try:
        with open(diag_json_path, "r", encoding="utf-8") as f:
            ai_diag_obj = json.load(f)
    except Exception as e:
        print(f"讀取 diagnosis_result.json 失敗: {e}")

with open(data_js_path, "w", encoding="utf-8") as f:
    f.write("// 自動生成的總經數據資料庫\n")
    f.write("var MACRO_DATA = " + json.dumps(timeseries_dict, ensure_ascii=False) + ";\n")
    f.write("var LATEST_SUMMARY = " + json.dumps(summary_list, ensure_ascii=False) + ";\n")
    f.write("var AI_DIAGNOSIS = " + json.dumps(ai_diag_obj, ensure_ascii=False) + ";\n")
